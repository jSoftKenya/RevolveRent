...
const GET_ALL_AGENCY_AGENTS= "GET_ALL_AGENCY_AGENTS";
...
export const fetchAdvert = id => dispatch => {
  axios
    ...
    .then(res => {
      console.log("Advert data:"+JSON.stringify(res.data));
      dispatch(advertFetchSuccess(res.data));
    })
    .catch(err => {
      console.log("Advert Error:"+JSON.stringify(err));
      dispatch(newError(err.response));
    });
};
...
export const getAllAgencyAgents = () => (dispatch, getState) => {
  const { userReducer } = getState();
  const { jwt } = userReducer;
  axios.defaults.headers.common["Authorization"] = `Bearer ${jwt}`;

  axios
    .get(`${baseUrl}/agency/all`)
    .then(response => {
      dispatch(getAllAgencyAgents(response.data));
    })
    .catch(err => dispatch(newError(err.response)));
};
