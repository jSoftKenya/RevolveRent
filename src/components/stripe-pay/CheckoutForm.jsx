...
export default function CheckoutForm(props) {
  ...
  useEffect(() => {
    if (userReducer) {
      if (!state.amountInShillings) {
        state.amountInShillings = +props.amountInShillings;
        ...
        axios
          .get(`${baseUrl}/payment/${state.amountInShillings}`)
          .then(res => {
  ...
  return (
    <Fragment>
      <div className="alert alert-info mt-3" role="alert">
        You are going to top up your account for {state.amountInShillings / 100}{" "}
        EUR. It is {state.amountInShillings / 100} new advertisements. 1
