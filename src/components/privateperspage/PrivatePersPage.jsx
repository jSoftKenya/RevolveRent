import React, { Component } from "react";
import { connect } from "react-redux";
import { Link } from "react-router-dom";
import PaymentWraper from "../pay/PaymentWraper";
import Moment from "react-moment";
import AddNewAdvert from "../addnewadvert/AddNewAdvert";
import statusCards from '../../assets/JsonData/status-card-data.json'

import StatusCard from "../status-card/StatusCard";

import RentCard from "../payrentcard/RentCard";

import { Stack, Button, Divider, Typography } from '@mui/material';



import axios from "axios";
import baseUrl from "../../utils/common";

const initialWalletBalance=0;



const walletDetails = {
  icon: "bx bx-receipt",
  amount: 0,
  title: "Wallet Balance",
};

let rentDetails = {
  icon: "bx bx-receipt",
  advertId: 1,
  amount: 11000,
  payDate: "5th, Monthly",
  title: "Monthly Rent",
};

const initialState = {
  topUp: false,
  success: false,
  amountInUSD: 0,
  error: "",
  walletBalance:0,
  myRentRequests: []
};

class PrivatePersPage extends Component {
  state = initialState;


  

  topUp = () => {
    if (this.state.amountInUSD < 1) {
      this.setState({
        error: "Please input value in Ksh.."
      });
      return;
    }
    this.setState({
      topUp: true,
      success: false,
      error: ""
    });
  };

  handleChange = e => {
    this.setState({
      [e.target.name]: e.target.value
    });
  };

  fetchRentRequests = () => {
    console.log("Requesting rent reuqsts for:"+this.props.user.id);
    const userId = this.props.user.id;
    axios
    .get(`${baseUrl}/rentrequest/myrequests/${userId}`)
    .then(res => {
      console.log("Rent Requests data:"+JSON.stringify(res.data));
      this.setState({ ...this.state, myRentRequests: res.data});
    })
    .catch(err => {
      console.log("RentRequests Error:"+JSON.stringify(err));
    });
  };

  componentDidMount() {
    this.fetchRentRequests();
  }


  componentDidUpdate(prevProps) {
    
    if (prevProps.user.paidAdvertLimit !== this.props.user.paidAdvertLimit) {
      this.setState({ ...this.state, success: true });
    }
  }

  
 

  render() {
    const favoriteAdverts = this.props.user.advertUserLikes.length;
    return (
      <div className="row mt-3">
        <div className="col-12 col-sm-12 col-md-12 col-xl-3">
          <div className="card">
            <div className="card-body">
              <h4>Welcome</h4>
              <h5 className="card-title">{this.props.user.username}</h5>
              <ul className="list-group list-group-flush">
                {+this.props.user.freeAdvertLimit ? (
                  <li className="list-group-item">
                    You can publish{" "}
                    <span className="font-weight-bold">
                      {this.props.user.freeAdvertLimit}
                    </span>{" "}
                    free advertisements
                  </li>
                ) : (
                  ""
                )}
                {+this.props.user.paidAdvertLimit ? (
                  <li className="list-group-item">
                    You can publish{" "}
                    <span className="font-weight-bold">
                      {this.props.user.paidAdvertLimit}
                    </span>{" "}
                    paid advertisements
                  </li>
                ) : (
                  ""
                )}
                <li className="list-group-item">
                  Registered:{" "}
                  <Moment fromNow ago>
                    {this.props.user.createdAt}
                  </Moment>
                </li>
                <li className="list-group-item">
                  Email: {this.props.user.email}
                </li>
                {+this.props.user.phoneNumber ? (
                  <li className="list-group-item">
                    Phone: {this.props.user.phoneNumber}
                  </li>
                ) : (
                  <li className="list-group-item">
                    You have not published your phone number.
                  </li>
                )}
                <li className="list-group-item">
                  <Link to="/favorites">
                    You have{" "}
                    <span className="font-weight-bold">{favoriteAdverts}</span>{" "}
                    favorite adverts
                  </Link>
                </li>
              </ul>
            </div>
          </div>
        </div>

        <div className="col-12 col-sm-12 col-md-12 col-xl-9">
                <div className="row">
                    <div className="col-6">
                            <RentCard
                                icon={"bx bx-receipt"}
                                amount={rentDetails.amount}
                                title={rentDetails.title}
                                payDate={rentDetails.payDate}
                                userId= {this.props.user.id}
                                advertId = "1"
                            />
                      </div>


                     <div className="col-3">
                          <StatusCard
                              icon={"bx bx-receipt"}
                              count= {+this.props.user.freeAdvertLimit ?  this.props.user.freeAdvertLimit : "0"}
                              title={"Free Ads Limit"}
                          />
                          
                     </div>     

                    
                    <div className="col-3">
                          <StatusCard
                              icon={"bx bx-receipt"}
                              count={walletDetails.amount}
                              title={walletDetails.title}
                          />
                          
                     </div>
                    
                     </div>

                     <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ my: 2 }}>
                     
                     
                       <StatusCard
                        icon={"bx bx-receipt"}
                        count={0}
                        title={"Transactions"}
                        to="/mytransactions"
                        />

                      <StatusCard
                                icon={"bx bx-receipt"}
                                count={2}
                                title={"My Bookings"}
                                to="/appointment"
                          />
                      
                      <StatusCard
                              icon={"bx bx-receipt"}
                              count={this.state.myRentRequests.length}
              
                              title={"Rent Requests"}
                              to="/myrequests"
                        />
                       <StatusCard
                          icon={"bx bx-receipt"}
                          count={"0"}
                          title={"Service Requests"}
                          to="/requestservice"
                        />
                        </Stack>
                        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ my: 2 }}>
                     
                        <StatusCard
                          icon={"bx bx-receipt"}
                          count={"0"}
                          title={"Complaints/Enquiry"}
                          to="/contactus"
                        />
                     </Stack>
         
         

              <Divider sx={{ my: 3 , mx: 3}}>
                <Typography variant="h5" sx={{ color: 'text.secondary' }}>
                  List a new property for rent/sale
                </Typography>
              </Divider>
             <div className="card row">
                 <AddNewAdvert />

            </div>
        </div>


        <div className="row">
          <div className="card">
            <div className="card-body">
          
              {this.state.success ? (
                <div className="alert alert-success mt-3" role="alert">
                  Your account was successfuly updated
                </div>
              ) : this.state.topUp ? (
                 <PaymentWraper amountInShillings={+this.state.amountInUSD * 100} />
  
              ) : (
                
                <div className="row">
                  <div className="input-group mb-3 col-12 col-sm-12 col-lg-5 col-xl-5">
                    <div className="input-group-prepend">
                      <span className="input-group-text">Ksh.</span>
                    </div>
                    <input
                      type="number"
                      className="form-control"
                      name="amountInUSD"
                      placeholder=""
                      min="1"
                      max="100"
                      step="1"
                      value={this.state.amountInUSD}
                      onChange={this.handleChange}
                    />
                    <div className="input-group-append">
                      <span className="input-group-text">.00</span>
                    </div>
                  </div>
                  {this.state.error ? (
                    <div className="input-group mb-3">
                      <small className="text-danger col-12">
                        {this.state.error}
                      </small>
                    </div>
                  ) : (
                    ""
                  )}
                  <div className="input-group mb-3">
                    <button
                      onClick={this.topUp}
                      className="btn btn-sm btn-success ml-3"
                    >
                      Top Up Balance
                    </button>
                  </div>
                </div>
              )}

            </div>
          </div>

        

        </div>
      </div>
    );
  }
}

function mapStateToProps(state) {
  if (state.userReducer) {
    return {
      user: state.userReducer.user
    };
  }
}

export default connect(mapStateToProps)(PrivatePersPage);
